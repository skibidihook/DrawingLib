...
function UI:CreateDrawing(class, props)
    local drawObj = Drawing.new(class)
    for prop, val in pairs(props) do
        drawObj[prop] = val
    end
    table.insert(self.Drawings, drawObj)
    return drawObj
end

function UI:IsMouseOver(drawingObj)
        ...
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            for _, elem in ipairs(window.Elements) do
                ...
                if elem.DropdownList then
                    for _, dObj in ipairs(elem.DropdownList) do
                        dObj.Position = dObj.Position + delta
                    end
                end
            end
        end
    ...
    function window:AddSlider(name, min, max, default, callback)
        ...
        UserInputService.InputChanged:Connect(function(input)
            if draggingSlider and input.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                slider.SliderFill.Size = Vector2.new(
                    slider.Background.Size.X * rel,
                    slider.Background.Size.Y
                )
                            ...
                            UserInputService.InputBegan:Connect(function(input2)
                                if input2.UserInputType == Enum.UserInputType.MouseButton1 then
                                    if UI:IsMouseOver(optBox) then
                                        ...
                                        for _, item in ipairs(dd.DropdownList) do
                                            item.Visible = false
                                        end
                                        dd.DropdownList = {}
                                    end
